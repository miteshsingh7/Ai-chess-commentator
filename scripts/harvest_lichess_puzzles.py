"""Harvest and cache tactical puzzles from Lichess Open Database via HuggingFace Datasets Server."""
...
OUTPUT_PATH = "data/lichess_tactical_puzzles.json"

TARGET_QUOTAS = {
    "fork": 20,
    "pin": 16,
    "skewer": 15,
    "discovered_attack": 15,
    "back_rank_mate": 10,
    "checkmate": 20,
    "trapped_piece": 15,
}
...
def main():
        ...
        try:
            with open(OUTPUT_PATH, "r", encoding="utf-8") as f:
                ...
                for item in existing:
                    collected[item["category"]].append(item)
                print(f"Loaded {len(existing)} existing harvested puzzles.")
    ...
    while any(len(collected[k]) < TARGET_QUOTAS[k] for k in TARGET_QUOTAS) and (offset // page_size) < max_pages:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": f"ChessCommentatorHarvest/1.0 (offset={offset})"})

        try:
            with urllib.request.urlopen(req) as resp:
                data = json.loads(resp.read().decode("utf-8"))
        except Exception as e:
            print(f"Fetch failed at offset {offset}: {e}. Backing off 8s...")
            time.sleep(8.0)
            offset += page_size
            continue
        ...
        for r in rows:
            ...
            # Board state when puzzle starts (after opponent's move)
            b = chess.Board(r["FEN"])
            b.push_uci(moves[0])
            fen_after_setup = b.fen()
            solver_move = chess.Move.from_uci(moves[1])

            # Classify using our taxonomy detector
        ...
        time.sleep(1.8)  # Polite pacing under HF limits
